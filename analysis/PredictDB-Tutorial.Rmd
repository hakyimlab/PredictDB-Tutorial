...
## Download data from [here](https://uchicago.app.box.com/s/ewnrqs31ivobz2sn6462cq2eb423dvpr)
...
Then we must turn this dataframe into a csv file to be used by the PEER tool and another one to be used as an input for the gtex pipeline
```{r}
...
Write out the residuals of the expression
```{r}
...
NB: The residuals expression file can be used for other analyses

Tutorial for gtex v7 pipeline:

Expression file: Use the transposed expression file as the input `gene_exp.csv`
Covariates: Use the covariates file `covariates.txt` as the inputs. If you have other covariates you want to regress out bind them to the `covariates.txt` ensuring the first column has the covariate identifier while the rest of the columns are the values for each individual. Each row represents a single covariate to be regressed out.
