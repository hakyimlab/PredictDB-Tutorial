...
This will create an RDS object out of the gene annotation file text file:
```{bash}
Rscript ./PredictDB-Scripts/geno_annot_to_RDS.R ./data/'gene_annot.parsed.txt' ./data/'gene_annot.RDS'
```

2. SNP Annotation - want to split this file by chromosome and turn them into .RDS format. We will end up with 22 separate .RDS files corresponding to the 22 chromosomes.

```{bash}
./PredictDB-Scripts/split_snp_annot_by_chr.py ./data/'geuvadis.annot.txt' ./data/'snp_annot'
```

Now we will turn all of these SNP annotation txt files into .RDS format so they can be used in the model.

```{bash}
Rscript ./PredictDB-Scripts/snp_annot_to_RDS.R ./data/'snp_annot.chr'
```

3. Genotype File - Want to also split this by chromosome:
```{bash}
./PredictDB-Scripts/split_genotype_by_chr.py ./data/'geuvadis.snps.txt' ./data/'genotype'
```
